...
const RowContainer = ({ flag, data, scrollValue }) => {
  ...
  const updateItems = (item) => {
    if (cartItems.some(i => i.title === item.title)) {
    ...
    >
      {
        data && data.length > 0 ?
          data.map(item => (
            <div
              key={item?.id}
              className='w-275 h-[190px] min-w-[275px] md:w-300 md:min-w-[300px]  bg-cardOverlay rounded-lg pt-2 pb-[40px] px-4  my-12 backdrop-blur-lg
